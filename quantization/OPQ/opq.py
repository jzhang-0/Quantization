...
class OPQ(object):
    ...
    def fit(self, vecs, pq_iter=20, rotation_iter=10, seed=123):
        ...
        for i in range(rotation_iter):
            ...
            if i == rotation_iter - 1:
                ...
                pq_tmp.fit(X, iter=pq_iter, seed=seed, par=0)
            else:
                # During the training for OPQ, just run one-pass (iter=1) PQ training
                pq_tmp.fit(X, iter=1, seed=seed, par=0)
                ##BUG joblib 有bug,无法开启并行，内存占用太多？

            # (b) Update a rotation matrix R
            X_ = pq_tmp.decode(pq_tmp.encode(X, par=0))
    ...
    def encode(self, vecs):
        ...
        """
        return self.pq.encode(self.rotate(vecs), par=0)
